...
def gradient_check(x, theta, epsilon = 1e-7):
    ...
    J_plus = forward_propagation(theta=thetaplus,x=x)                                  # Step 3
    J_minus = forward_propagation(theta=thetaminus,x=x)                                 # Step 4
    gradapprox = (J_plus - J_minus) / (2 * epsilon)                              # Step 5
    ### END CODE HERE ###
    
    # Check if gradapprox is close enough to the output of backward_propagation()
    ### START CODE HERE ### (approx. 1 line)
    grad = backward_propagation(theta=theta,x=x)
    ### END CODE HERE ###
...
def gradient_check_n(parameters, gradients, X, Y, epsilon = 1e-7):
    ...
    for i in range(num_parameters):
        ...
        J_plus[i], _ = forward_propagation_n(X=X,Y=Y,parameters=vector_to_dictionary(thetaplus))                                   # Step 3
        ### END CODE HERE ###
        ...
        J_minus[i], _ = forward_propagation_n(X=X,Y=Y,parameters=vector_to_dictionary(thetaminus))                                  # Step 3
        ### END CODE HERE ###
